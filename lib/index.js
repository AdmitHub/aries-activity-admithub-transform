...
export default class AdmitHubTransform extends Activity {
    ...
    transformSA_Appl_87(contact) {
        const attribute = find(contact.attributes, (obj) => {
            return obj.mappingName === 'SA_Appl_87';
        });

        if (!attribute || attribute.value === '87' || attribute.value === '') {
            contact.georgia.successAcademy = { ...contact.georgia.successAcademy, qualified: true };
            return contact;
        }

        let georgia = { ...contact.georgia };
        georgia.successAcademy = { ...georgia.successAcademy, qualified: value };
        return { ...contact, georgia };
    }

    transformSA_Appl_Received(contact) {
        const attribute = find(contact.attributes, (obj) => {
            return obj.mappingName === 'SA_Appl_Received';
        });

        if (!attribute || attribute.value === 'N') {
            contact.georgia.successAcademy = { ...contact.georgia.successAcademy, appReceived: false };
            return contact;
        }

        if (!attribute || attribute.value === '') {
            contact.georgia.successAcademy = { ...contact.georgia.successAcademy, appReceived: value };
            return contact;
        }
        const value = attribute.value === 'Y';
        let georgia = { ...contact.georgia };
    ...
    transformSuccess_Academy(contact) {
        const attribute = find(contact.attributes, (obj) => {
            ...
            if (!attribute || attribute.value === '') {
                contact.georgia.successAcademy = { ...contact.georgia.successAcademy, accepted: value };
                return contact;
            }
            const value = attribute.value === 'Y';
            let georgia = { ...contact.georgia };
            georgia.successAcademy = { ...georgia.successAcademy, accepted: true };
            return { ...contact, georgia };
    ...
    transformTuitionPaymentPlan(contact) {
        const attribute = find(contact.attributes, (obj) => {
            ...
            if (!attribute || attribute.value === 'N') {
                contact.georgia.successAcademy = { ...contact.georgia.successAcademy, paymentPlan: false };
                return contact;
            }

            if (!attribute || attribute.value === '') {
                contact.georgia.successAcademy = { ...contact.georgia.successAcademy, paymentPlan: value };
                return contact;
            }
            const value = attribute.value === 'Y';
            let georgia = { ...contact.georgia };
            georgia.successAcademy = { ...georgia.successAcademy, paymentPlan: true };
            return { ...contact, georgia };
    }
    ...
    transformWorkstudyAwardAmount(contact) {
        ...
        georgia.finAid.hsCode = attribute.value;
